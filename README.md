...
## Optional models (OpenNyAI Legal NER + IndicNER)

These models are **optional**. The app works without them.

- If you install them: better name/entity detection in many legal/Indian documents.
- If you skip them: app still runs using base detection.

### Important (Windows)
Install optional models in the **same terminal/environment** where backend dependencies were installed.
...
If you did not create `.venv`, you can still run the install commands from your backend terminal.

### Option A: Install OpenNyAI Legal NER only

```bat
pip install https://huggingface.co/opennyaiorg/en_legal_ner_trf/resolve/main/en_legal_ner_trf-any-py3-none-any.whl
```

### Option B: Install IndicNER only

```bat
pip install transformers torch
```

### Option C: Install both (recommended for best coverage)
Run both commands above.

### Verify installation (safe to run)

```bat
python -c "import spacy; spacy.load('en_legal_ner_trf'); print('OpenNyAI OK')"
python -c "from transformers import pipeline; pipeline('ner', model='ai4bharat/IndicNER'); print('IndicNER OK')"
```

If a command prints `... OK`, that model is available.

### After installing optional models
1. Close backend terminal window.
2. Start again with `start_windows.bat`.
3. Re-run your document in the UI.

### If installation fails
- This does **not** block app usage. You can continue without optional models.
- Common fixes:
  1. Ensure Python version is 3.10–3.13.
  2. Upgrade pip in your active environment:
     ```bat
     python -m pip install --upgrade pip
     ```
  3. Retry the model install command.
